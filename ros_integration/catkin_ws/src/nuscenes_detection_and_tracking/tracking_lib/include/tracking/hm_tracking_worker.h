...
public:
    ...
    virtual inline const std::map<IdType, Trajectory>& collectTrajectories() const { return trajectory_poses_; }

    virtual inline const std::vector<ObjectPtr>& collectTrackingObjectsInWorld() const
    {
    ...
    virtual std::vector<FixedTrajectory> collectFixedTrajectories()
    {
        ...
        fixedTrajectories.reserve(trajectory_periods_.size());

        for (auto iter = trajectory_periods_.begin(); iter != trajectory_periods_.end();) {
            if (iter->second > 0.0) {
                fixedTrajectories.emplace_back(iter->first, iter->second);
    ...
    virtual double getLatestTrackingTimestamp() const { return time_stamp_; }
     ...
     * @return
     */
    virtual std::vector<ObjectPtr> collectExpectedObjects(const double& timestamp,
                                                          const Eigen::Matrix4d& trans_pose) const
    {
        ...
        if (booted_) {
            const std::vector<ObjectTrackerPtr>& trackers = multi_object_tracker_->getTrackers();
            ...
            for (size_t i = 0u; i < trackers.size(); ++i) {
                ...
                if (time_diff == 0) {
                    expected_obj->clone(*(trackers[i]->current_object_));
                }
                else {
                    trackers[i]->getExpectedObject(time_diff, expected_obj);
                }
                obj->clone(*(expected_obj->object_ptr));
                // Convexhull polygon is not cloned above, manually copy over
                pcl::copyPointCloud<PointD, PointD>(expected_obj->object_ptr->polygon, obj->polygon);
                ...
                dir = (pose_world2velo * Eigen::Vector4d(dir[0], dir[1], dir[2], 0)).head(3);

                Eigen::Vector3d coord_dir(1.0, 0.0, 0.0);
                obj->yaw_rad = common::geometry::computeTheta2dXyBetweenVectors<Eigen::Vector3d>(
                ...
                center = (pose_world2velo * Eigen::Vector4d(center[0], center[1], center[2], 1)).head(3);
                ...
                vel = (pose_world2velo * Eigen::Vector4d(vel[0], vel[1], vel[2], 0)).head(3);

                obj->velocity_uncertainty = expected_obj->velocity_uncertainty;

                // restore original local Velodyne coordinates
                // 世界坐标-->局部坐标
                common::transform::transformPointCloud<PointI>(pose_world2velo, obj->cloud);
                common::transform::transformPointCloud<PointD>(pose_world2velo, obj->polygon);
...
protected:
    ...
    // @params[IN] options: tracker options with necessary information
    // @return nothing
    void constructTrackedObjects(const std::vector<ObjectPtr>& objects_obsved,
    ...
    // @params[IN] pose: pose using for coordinate transformation
    // @return nothing
    void transformTrackedObject(TrackableObjectPtr object_obsved, const Eigen::Matrix4d& pose);
    //--------------------- construct trackable objects ----------------------//
    ...
    /// @brief Update track history data structures
    void updateTrackHistory(const std::vector<ObjectPtr>& tracked_objs);
...
private:
    ...
    std::shared_ptr<feature::BaseFeatureExtractor> feature_extractor_;
